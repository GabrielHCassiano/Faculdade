...
/* 1. Recuperar o nome e telefone dos clientes, o número de contrato de locação de veículo, além do modelo eplaca do veículo.
   Ordene os contratos pela data de saída mais recente */

SELECT c.nome, t.numero, con.numero, con.dataSaida, v.placa ,v.modelo 
FROM CLIENTE c INNER JOIN TELEFONE t ON c.codigo = t.codigoCliente 
INNER JOIN CONTRATO con ON c.codigo = con.codigoCliente 
INNER JOIN VEICULO v ON con.idVeiculo = v.id 
ORDER BY con.dataSaida DESC;

-- 2. Mostrar a cidade de cada filial, seu ano de fundação e o “Total de veículos” de cada filial específica.

SELECT f.cidade, ci.anoFundacao, COUNT(v.id) AS 'TotalVeículos' FROM CIDADE Ci JOIN FILIAL f ON ci.codigo = f.idCidade 
JOIN VEICULO v ON f.codigo = v.codFilial GROUP BY f.cidade, ci.anoFundacao ORDER BY f.cidade;

/* 3. Mostrar todos os contratos firmados pelo cliente “Filomena” sobre o veículo de placa “'MAB0002'”.
   Mostrar o “Nome do cliente”, “Número do contrato”, “Placa do veículo”, “Modelo do veículo”, “Data saída” e“Data devolução” */
   
SELECT c.nome AS 'Nome do cliente', con.numero AS 'Número do contrato', v.placa AS 'Placa do veículo', v.modelo 
AS 'Modelo do veículo', con.dataSaida AS 'Data saída' , con.dataDevolucao AS 'Data devolução' 
FROM CLIENTE c INNER JOIN TELEFONE t ON c.codigo = t.codigoCliente 
INNER JOIN CONTRATO con ON c.codigo = con.codigoCliente 
INNER JOIN VEICULO v ON con.idVeiculo = v.id 
AND c.nome = 'Filomena' AND v.placa = 'MAB0002';
   
/* 4. Exibir a placa e o modelo de todos os veículos de 4 portas das filiais cuja população seja inferior a cem milhabitantes.
   Exibir também os nomes das cidades das filiais e as Ruas. */

SELECT f.cidade, f.rua, v.placa, v.modelo, v.numPortas FROM CIDADE ci JOIN FILIAL f ON ci.codigo = f.idCidade 
JOIN VEICULO v ON f.codigo = v.codFilial WHERE ci.populacao < 100000 AND v.numPortas = 4;
    
/* 5. Mostrar a quantidade de veículos com diárias maiores que 100 reais e registrados em filiais com populaçãomaior que 500 mil habitantes.
   Mostre o modelo do veículo e a quantidade. */

SELECT v.modelo, COUNT(*) FROM CIDADE ci JOIN FILIAL f ON ci.codigo = f.idCidade JOIN VEICULO v ON f.codigo = v.codFilial 
WHERE v.valorDiaria > 100 AND ci.populacao > 500000 GROUP BY v.modelo ORDER BY v.modelo;
   
-- 6. Mostrar a média do valor recebido por alocações feitas em cada veículo. Ordene estes valores em ordemdecrescente.

SELECT v.placa, v.modelo, AVG(con.valorPago) AS 'Média do Valor' 
FROM FILIAL f JOIN VEICULO v ON f.codigo = v.codFilial JOIN CONTRATO con ON v.id = con.idVeiculo 
GROUP BY v.id ORDER BY AVG(con.valorPago) DESC;

-- 7. Exibir a placa, o modelo e o valor da diária do(s) veículo(s) que tiveram o maior valor de multa aplicadanuma locação.

SELECT v.placa, v.modelo, v.valorDiaria FROM VEICULO v JOIN CONTRATO con ON v.id = con.idVeiculo 
WHERE con.multa = (SELECT MAX(multa) FROM CONTRATO);

-- 8. Exiba o modelo e a placa dos veículos que ainda não foram locados na empresa.

SELECT v.modelo, v.placa FROM VEICULO v WHERE v.id NOT IN (SELECT DISTINCT idVeiculo FROM CONTRATO);